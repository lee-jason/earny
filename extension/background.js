...
chrome.storage.local.get("earnyWaivedMode").then((data) => {
    ...
    if (savedSession && savedSession.minutes > 0) {
      ...
      if (result.error) {
        ...
        if (result.error !== "Insufficient balance" && result.error !== "Unauthorized") {
          await chrome.storage.local.set({ earnyUncommittedSession: savedSession });
        }
      } else {
  ...
  (async () => {
    switch (message.action) {
      ...
      case "setWaivedMode":
        ...
        chrome.storage.local.set({ earnyWaivedMode: waivedMode });
